...
export class ActionGenerator {
  ...
  private actionForPurchase(
    records: (License | Transaction)[],
    event: PurchaseEvent
  ): Action {
    const recordsToSearch = [event.transaction, ...event.licenses].filter(
      isPresent
    );
    const deal = this.singleDeal(this.getDealsForRecords(recordsToSearch));
    if (deal) this.recordSeen(deal, event);

    const metaAction = this.maybeMakeMetaAction(
      event,
      deal,
      event.transaction?.data.vendorAmount ?? 0
    );
    if (metaAction) return metaAction;
  ...
  private actionForRenewal(
    records: (License | Transaction)[],
    event: RenewalEvent | UpgradeEvent
  ): Action {
    ...
    const metaAction = this.maybeMakeMetaAction(
      event,
      deal,
      event.transaction.data.vendorAmount
    );
    if (metaAction) return metaAction;
  ...
  private actionsForRefund(
    ...
    return [...deals]
      .map((deal) =>
        this.makeUpdateAction(records, deal, null, DealStage.CLOSED_LOST, {
          amount: 0,
        })
      )
      .filter(isPresent);
      ...
      if (record instanceof Transaction) {
        ...
        ids.add(
          record.data.addonLicenseId &&
            uniqueTransactionId(txId, record.data.addonLicenseId)
        );
        ids.add(
          record.data.appEntitlementId &&
            uniqueTransactionId(txId, record.data.appEntitlementId)
        );
        ids.add(
          record.data.appEntitlementNumber &&
            uniqueTransactionId(txId, record.data.appEntitlementNumber)
        );
      } else {
  ...
  private singleDeal(foundDeals: Set<Deal>) {
    ...
    if (foundDeals.size === 1) {
      ...
      if (this.dealManager.duplicates.has(dealToUse)) {
        throw new Error(
          `Primary duplicate is accounted for twice: ${dealToUse.id}`
        );
      }
  ...
  private makeUpdateAction(
    ...
    if (record) {
      Object.assign(
        deal.data,
        this.dealCreationProperties(
          records,
          record,
          dealStage ?? deal.data.dealStage
        )
      );
      deal.data.licenseTier = Math.max(
        deal.data.licenseTier ?? -1,
        record.tier
      );
    }
  ...
  private dealCreationProperties(
    ...
    return {
      ...
      dealName: mustache.render(
        this.dealPropertyConfig.dealDealName,
        record.data
      ),
      ...
      transactionId:
        record instanceof Transaction ? record.data.transactionId : null,
...
function getLatestLicense(event: PurchaseEvent): License {
  return [...event.licenses].sort(
    sorter((item) => item.data.maintenanceStartDate, "DSC")
  )[0];
}
