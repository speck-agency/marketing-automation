...
export class Deal extends Entity<DealData> {
  ...
  hasActivity() {
    return (
      ...
      isNonBlankString(
        this.downloadedData["engagements_last_meeting_booked"]
      ) ||
...
function makeAdapter(config: HubspotDealConfig): EntityAdapter<DealData> {
  ...
  function enumFromValue<T extends number>(
    ...
    if (!found)
      throw new AttachableError(
        "Cannot find ENV-configured mapping:",
  ...
  return {
    ...
    shouldReject(data) {
      ...
      if (config.attrs?.duplicateOf && data[config.attrs?.duplicateOf])
        return true;
      return false;
    },
    ...
    data: {
      ...
      maintenanceEndDate: {
        ...
        down: (maintenanceEnd) =>
          maintenanceEnd ? maintenanceEnd.substr(0, 10) : null,
